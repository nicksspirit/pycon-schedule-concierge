...
@st.cache_data
def load_area_latlng():
    return pd.read_csv(cfg.DATA_DIR / "area_latlong.csv", index_col="AREA_LABEL")
...
def build_graph_view(carrier_network: nx.DiGraph):
    # Update each node with a name attribute
    for area in carrier_network.nodes():
        carrier_network.nodes[area]["name"] = area
...
def build_map_view(carrier_network: nx.DiGraph, latlng_df: pd.DataFrame):
    fig = go.Figure()
    longitudes = []
    latitudes = []
    arrow_traces = []

    for orig_area, dest_area, data in carrier_network.edges(data=True):
...
if __name__ == "__main__":
    ...
    if not uploaded_file:
        ...
        area_latlng_df = load_area_latlng()
